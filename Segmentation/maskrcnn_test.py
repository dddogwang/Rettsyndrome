...
img_uint8 = img_uint8*250/np.max(img_uint8)
visualize.display_instances_save(img_uint8, F_total_boxes, F_total_masks, F_total_class_ids, ["BG","nucleus"], 
                                        savename=sys.argv[5]+"/"+imagename[:-8]+"_all.png", figsize=(20, 20))
print("save maskrcnn segmentation results as ", sys.argv[5]+"/"+imagename[:-8]+".png", flush=True)
for i in range(3):
    one = img_uint8[:,:,i].astype(np.uint8)
    zero = np.zeros(one.shape, dtype=np.uint8)
    img_uint8_onlyone = cv2.merge([zero, zero, zero])
    img_uint8_onlyone[:,:,i] = one
    img_uint8_onlyone = gamma_img(1.5, img_uint8_onlyone)
    visualize.display_instances_save(img_uint8_onlyone, F_total_boxes, F_total_masks, F_total_class_ids, ["BG","nucleus"], 
                                        savename=sys.argv[5]+"/"+imagename[:-8]+"_"+str(i)+".png", figsize=(20, 20))
    print("save maskrcnn segmentation results as ", sys.argv[5]+"/"+imagename[:-8]+"_"+str(i)+".png", flush=True)
